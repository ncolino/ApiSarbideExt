...
class Database:
    ...
    def ConsultaProjectListByUser(self, _rightframeAppid, _email):        
        self.cur.execute("""select PROJECT.PROJECT_ID, PROJECT.PROJECT_TYPE_ID, PROJECT.TITLE_01, DATE_FORMAT(NODE_GROUPUSER_MEDIA.NGM_CRE, '%%Y-%%m-%%d %%H:%%i:%%s') AS FECHA_CRE, USERS.USER_ID
            FROM PROJECT,NODE_GROUPUSER_MEDIA,GROUPUSER_USERS,USERS,NODEGROUPUSER_RIGHT_FRAME
            ...
            AND NODE_GROUPUSER_MEDIA.NODE_ID = NODEGROUPUSER_RIGHT_FRAME.NODE_ID
            AND NODEGROUPUSER_RIGHT_FRAME.RIGHT_FRAMEAPLICATION_ID = %s
            AND USERS.E_CORREO = %s ORDER BY FECHA_CRE DESC""", (_rightframeAppid, _email))
        #results = self.cur.fetchall()  --> usamos mejor el cursor        
        result = []
        for row in self.cur:            
            row['PROJECT_ID'] = row['PROJECT_ID'] 
            row['PROJECT_TYPE_ID'] = row['PROJECT_TYPE_ID'] 
    ...
    def ConsultaUserRightFramesAppByFrameAppUser(self,_frameAppid,_email):  
        self.cur.execute("""SELECT DISTINCT RIGHT_FRAMEAPLICATION.RIGHT_FRAMEAPLICATION_ID, RIGHT_FRAMEAPLICATION.FRAME_APLICATION_ID, RIGHT_FRAMEAPLICATION.NAME,RIGHT_FRAMEAPLICATION.WEB_URL,RIGHT_FRAMEAPLICATION.WEB_CLASS,RIGHT_FRAMEAPLICATION.WEB_TITLE 
...
class Metodos:    
    ...
    def funcion_GetProjectListByUser(self, _rightframeAppid, _email):        
        ...
        db = Database()  
        db.Connect_pymysql()
        proys = db.ConsultaProjectListByUser(_rightframeAppid, _email)    
        db.Disconnect()    
